...
import React, { ForwardedRef, forwardRef, ReactText } from 'react';
...
>(
  {
    ...
    scrollViewProps,
    onScroll,
    renderAheadOffset = SCREEN_HEIGHT,
    numColumns = 1,
    gap = 0,
    marginHorizontal = 0,
  ...
  const rowRenderer = (type: ReactText, item: T, index: number) => {
    ...
    return (
      <Box className="overflow-hidden" style={{ width, height }}>
        <Box
          className="flex-1"
          style={{
            paddingLeft,
...
function getContainerWidth(margin = 0) {
  return Math.round(SCREEN_WIDTH * 1000) / 1000 - margin * 2;
}
